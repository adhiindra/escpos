...
/**
 * [raster description]
 * @param  {Image} image         The image to print as raster
 * @param  {string} mode          The printing mode ('normal', 'dhdw', 'dwh', 'dhw')
 * @param  {number} [paddingLeft] Optional number of blank columns to add on the left
 * @return {Printer}              The escpos printer instance
 */
Printer.prototype.raster = function (image, mode, paddingLeft) {
  if (!(image instanceof Image)) {
    throw new TypeError('Only escpos.Image supported');
  }
  ...
  if (paddingLeft && paddingLeft > 0) {
    // Calculate the new width of the raster with padding
    var paddedWidth = width + paddingLeft;
    var paddedRasterData = Buffer.alloc(paddedWidth * height);

    // Copy raster data with padding
    for (var y = 0; y < height; y++) {
      for (var x = 0; x < width; x++) {
        var srcIndex = y * width + x;
        var destIndex = y * paddedWidth + paddingLeft + x;
        paddedRasterData[destIndex] = raster.data[srcIndex];
      }
    }

    // Update width to padded width
    width = paddedWidth;
    raster.data = paddedRasterData;
  }
